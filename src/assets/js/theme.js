/*! YOOtheme Pro v2.3.8 | https://yootheme.com */
!(function (t, i) {
  "use strict";
  function e(t) {
  ...
  var a,
    ...
    n = {
      update: [
        {
          read: function () {
            return !!d() && { height: this.$el.offsetHeight };
          },
          write: function (t) {
            var e = t.height,
              a = d(),
            ...
            i.hasClass(this.$el, "tm-header-transparent") ||
              ...
              i.css(i.$(".tm-header-placeholder", r), { height: e });
          },
          events: ["resize"],
        },
      ],
...
(function () {
  ...
  var ColorUtil = {
    lightOrDarkImage: function (url) {
      if (!colorThief) {
        colorThief = ajax(
          ...
          document.body.appendChild(
            document
              .createRange()
              .createContextualFragment("<script>" + xhr.response + "</script>")
          );
        });
      }
      ...
      return colorThief.then(function () {
        return getImage(url).then(function (img) {
          return ColorUtil.lightOrDark(
            "rgb(" + new ColorThief().getColor(img).join(",") + ")"
          );
        });
      });
    },
    ...
    lightOrDark: function (hexcolor) {
      return Promise.resolve(
        ColorUtil._rgbToHsl.apply(
          this,
          ColorUtil._parseColor(hexcolor).slice(0, 3)
        )[2] < 0.52
          ? "dark"
          : "light"
      );
    ...
    _parseColor: function (color) {
      ...
      // match #aabbcc
      if (
        (match = /#([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})/.exec(
          color
        ))
      ) {
        quadruplet = [
        ...
        (match = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(
          color
        ))
      ) {
        quadruplet = [
  ...
  if (style !== "default") {
    on(
      ...
      function (e) {
        ...
        var bgImage =
          $('> [style*="background-image"]', pageContainer) ||
          $('> [style*="background-image"]', section);
        var color;

        if (
          !bgImage ||
          css(bgImage, "background-image").replace(/(url\(|"|'|\))/g, "") ===
            "none"
        ) {
          if (pageContainer) {
            color = ColorUtil.lightOrDark(
              css(pageContainer, "background-color")
            );
          } else {
          ...
          !includes(
            ...
            css(bgImage, "background-color")
          )
        ) {
          color = ColorUtil.lightOrDark(css(bgImage, "background-color"));
        } else {
          color = ColorUtil.lightOrDarkImage(
            css(bgImage, "background-image").replace(/(url\(|"|'|\))/g, "")
          );
        }
        ...
        color.then(function (color) {
          if (color === modifier) {
            ...
            if (sticky) {
              ...
              comp.clsInactive = comp.$props.clsInactive = comp.clsInactive.replace(
                /dark|light/,
  ...
  ready(function () {
    ...
    $$(
      ".uk-modal-full .uk-nav.uk-nav-primary li, .uk-offcanvas-bar .uk-nav.uk-nav-primary li"
    ).forEach(function (li) {
    ...
    if (style !== "default") {
      $$('a[href]:not([href*="style="]):not(.uk-navbar-toggle)').forEach(
        function (a) {
          if (
  ...
  function getParam(key, uri) {
    var value = (new RegExp("[?&]" + key + "=([^&#]*)", "i").exec(uri) ||
      [])[1];
